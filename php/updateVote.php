...
error_reporting (E_ALL);
    ...
    $reqControle = $pdo->prepare("SELECT a_vote FROM presence WHERE id=(?)") or exit(print_r($pdo->errorInfo()));
    ...
    if($res == 1) echo "Vous avez déjà voté";
        ...
        $req = $pdo->prepare("UPDATE presence SET a_vote=(?) WHERE id=(?)") or exit(print_r($pdo->errorInfo()));
        $req->execute(array($a_vote,$ligne));

        //Enregistrement dans la table votes
        $reqVote = $pdo->prepare("INSERT INTO votes (num_match, num_vote) VALUES (?,?)") or exit(print_r($pdo->errorInfo()));
